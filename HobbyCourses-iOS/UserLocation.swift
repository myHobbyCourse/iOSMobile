...
extension UserLocation: CLLocationManagerDelegate {
        ...
        geoLocation.reverseGeocodeLocation(location, completionHandler: { (placeMarks, error) -> Void in
            if let pmark = placeMarks, pmark.count > 0 {
                ...
                if let addr = place.addressDictionary {
                    print("The address dictionary : \(place.addressDictionary)")
                    
                    let subLocality = addr["SubLocality"] as? String
                    let cityName = addr["City"] as! NSString?
                    let countryName = addr["Country"] as! NSString?
                    var locationString :String = ""
                    if let subLocal = subLocality {
                        locationString += subLocal as String
                    }
    ...
    func pincodeFromlocation(_ location: CLLocation, block: @escaping (String?)->()){
        let geoLocation = CLGeocoder()
        geoLocation.reverseGeocodeLocation(location, completionHandler: { (placeMarks, error) -> Void in
            if let pmark = placeMarks, pmark.count > 0 {
                let place :CLPlacemark = pmark.last! as CLPlacemark
                if let pin = place.postalCode{
                    block(pin)
                }else{
                    block(nil)
                }
    ...
    func cityFromlocation(_ location: CLLocation, block: @escaping (String?)->()){
        ...
        geoLocation.reverseGeocodeLocation(location, completionHandler: { (placeMarks, error) -> Void in
            if let pmark = placeMarks, pmark.count > 0 {
                let place :CLPlacemark = pmark.last! as CLPlacemark
                if let addr = place.addressDictionary,let city = addr["City"] as? String {
                    block(city)
                }else{
                    block(nil)
                }
...
/*
 ...
 print("Got error while getting location: \(error.localizedDescription)")
 ...
 */
 ...
 print("Monitoring region has been Compramized to maximum of location manager")
 }
 ...
 print("Got Error while monitoring region: \(error.localizedDescription)")
 }
 
 func locationManager(manager: CLLocationManager, didStartMonitoringForRegion region: CLRegion) {
 print("Monitoring Starter for region with identifier: \(region.identifier)")
 }
 
 // MARK: - Test Cases
 func testRegionEntranceForAnybox() {
 let key = "VmWJTOSwHXKwSf6z"
 print("Testing: User did enter the region for key : \(key)")
 testBlock?(key)
 }
